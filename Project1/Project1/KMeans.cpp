...
void SaveClustersToFile(const char* filename, double* data, int* labels, int size, int dim)
{
	ofstream outfile;
	...
	for (int i = 0; i < size; ++i) {
		// �������ݵ�
		for (int j = 0; j < dim; ++j) {
			outfile << data[i * dim + j];
			if (j < dim - 1) {
				outfile << ", ";  // ��ÿ������ֵ֮��Ӷ���
			}
		}
		// ����������cluster
		outfile << " -> Cluster " << labels[i] << endl;
	}

	outfile.close();
	cout << "Data and cluster labels have been saved to " << filename << endl;
}