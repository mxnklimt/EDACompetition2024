...
#include "ffdot.h" 
#include "isOverlap.h" 
std::pair<int, int> findNonOverlappingPosition(int center_x, int center_y, int buff_width, int buff_height, const std::vector<std::vector<Point>>& clusters,int x_max,int x_min,int y_max,int y_min,std::vector<std::vector<Point>>& clusters_buff1, std::vector<std::vector<Point>>& clusters_buff2,int buff_flag){
            ...
            for (int dx = -radius; dx <= radius; dx += step) {//两层for循环遍历边长为radius的正方形，搜索步长精度step
                for (int dy = -radius; dy <= radius; dy += step) {
                    ...
                    //这个 lambda 函数将在一个独立的线程中执行。    
                        ...
                        if ((!isOverlap(new_x, new_y, buff_width, buff_height, clusters))&& (!isOverlap(new_x, new_y, buff_width, buff_height, clusters_buff1)) && (new_x>x_min) && (new_x < x_max) && (new_y > y_min) && (new_y < y_max)&&( (buff_flag!=2) || (!isOverlap(new_x, new_y, buff_width, buff_height, clusters_buff2))) ) {
